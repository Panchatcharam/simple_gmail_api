...
#if defined(_MSC_VER)
# include <io.h>
# define S_IRUSR int(_S_IREAD)
# define S_IWUSR int(_S_IWRITE)
# define S_IXUSR int(_S_IEXEC)
# define S_IRWXU int(_S_IREAD | _S_IWRITE | _S_IEXEC)
# define S_IRWXG int(0)
# define S_IRWXO int(0)
...
class FileOpenOptions {
 public:
  ...
  void set_permissions(int bits) { permissions_ = bits; }
...
class File {
 public:
  ...
  static googleapis::util::Status RecursivelyCreateDirWithPermissions(
      const string& path, int permissions);
